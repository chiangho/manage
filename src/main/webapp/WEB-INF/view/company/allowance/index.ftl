...
<div id="addCompanyItemWindow" style="display: none;text-align: center;width: 100%;padding:0px;">
    <div class="easyui-layout" data-options="fit:true" style="width: 100%;height:360px">
	    ...
	    <div data-options="region:'east'" style="width:70%;text-align: center;padding-top:10px;">
		    <form id="addForm" method="post" action="${domain}company/allowance/save.action">
				...
				<div class="form-item">
					<label for="name" class="label-top">津贴名称：</label>
					<input id="name"  name="name" class="easyui-validatebox easyui-textbox" style="width:250px" prompt="请输入津贴名称" >
				</div>
				<div class="form-item">
					<label for="summayScript" class="label-top">脚本：</label>
...
<div id="buttonArea">
	<a href="javascript:openaddCompanyItemWindow()" class="easyui-linkbutton primary" data-options="iconCls:'fa fa-plus'">添加津贴</a>
</div>
...
function loadBaseData(companyId){
	...
	hao.ajax({
		url:"${domain}company/allowance/load_sun_dataitem.action",
		dataType:"text",
		data:{
			companyId:companyId
		},
...
function del(id){
	$.messager.confirm("提供","是否删除此数据项目",function(r){
		if(r){
			hao.ajax({
				url:"${domain}company/allowance/del.action",
				data:{
					id:id
				},
				success:function(rtn){
...
function openEdit(id,companyId){
	...
	$("#addForm").form("load","${domain}company/allowance/load_company_data.action?id="+id);
}
/**
 * 保存
 */
...
function viewScript(id){
	hao.ajax({
		url:"${domain}company/allowance/load_company_data.action",
		data:{
			id:id
		},
		success:function(rtn){
...
$(document).ready(function(){
	...
	$("#list").datagrid({
		url: '${domain}company/allowance/page_list.action',
		cache : false,
		rownumbers : true,
		pagination : true,
		toolbar: $("#buttonArea"),
