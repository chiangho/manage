...
<div id="addCompanyItemWindow" style="display: none;text-align: center;width: 100%;padding:0px;">
    <div class="easyui-layout" data-options="fit:true" style="width: 100%;height:360px">
	    ...
	    <div data-options="region:'east'" style="width:70%;text-align: center;padding-top:10px;">
		    <form id="addForm" method="post" action="${domain}company/salary_item/save.action">
...
function loadBaseData(companyId){
	...
	hao.ajax({
		url:"${domain}company/salary_item/load_sun_dataitem.action",
		dataType:"text",
		data:{
			companyId:companyId,
			selfId:$("#id").val()
...
function del(id){
	$.messager.confirm("提供","是否删除此数据项目",function(r){
		if(r){
			hao.ajax({
				url:"${domain}company/salary_item/del.action",
				data:{
					id:id
				},
				success:function(rtn){
...
function openEdit(id,companyId){
	...
	$("#addForm").form("load","${domain}company/salary_item/load_company_data.action?id="+id);
}
/**
 * 保存
 */
...
function viewScript(id){
	hao.ajax({
		url:"${domain}company/salary_item/load_company_data.action",
		data:{
			id:id
		},
		success:function(rtn){
...
$(document).ready(function(){
	...
	$("#list").datagrid({
		url: '${domain}company/salary_item/page_list.action',
		cache : false,
		rownumbers : true,
		pagination : true,
		toolbar: $("#buttonArea"),
		columns:[[
			...
			{field:"name",title:"工资项",width:200},
			{field:"operation",title:"性质",width:200,formatter:function(value,row,index){
				if(value=="0"){
					return "扣款";
				}else{
					return "加款";
