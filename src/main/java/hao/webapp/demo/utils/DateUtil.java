...
public class DateUtil {
	...
	public static int getWorkDayNum(Date beginDate, Date endDate) {
		...
		startCalender.setTime(beginDate);
		String httpArg = "";
		int i = 0;
		while (endCalender.getTime().getTime() > startCalender.getTime().getTime()) {
			if (i == 0) {
				httpArg += f.format(startCalender.getTime());
			} else {
				httpArg += "," + f.format(startCalender.getTime());
			}
			startCalender.add(Calendar.DAY_OF_YEAR, 1);
			i++;
		}
		...
		// 0 上班 1周末 2节假日
		int num = 0;
		for (String day : httpArg.split(",")) {
			System.out.println(day + ":" + json.getInt(day));
	...
	/**
	 * http://www.cnblogs.com/0201zcr/p/5000977.html date2比date1多的天数
	 *
	 * @param date1
	 * @param date2
	 * @return
	...
	public static void main(String[] args) throws ParseException {
		...
		int  n = getYearDiff(f.parse("2005-05-01"), f.parse("2006-04-01"));
		System.out.println("两个日期相隔:"+ n + "年");
	}
}
